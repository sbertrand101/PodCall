...
var Router = function (id, token, secret) {
	...
	var wtfNum = process.evn.WTF_NUM;
	var bsReportNum = process.env.BS_REPORT_NUM;
	var securityNowNum = process.env.SECURITY_NUM;

	//podcast URLs
	var thisAmericanLife;
	catapult.Numbers.get(client, thisAmericanLifeNum, function (err, number) {
		thisAmericanLife = number.name;
	});
	var planetMoney;
	catapult.Numbers.get(client, planetMoneyNum, function (err, number) {
		planetMoney = number.name;
	});
	var tedRadio;
	catapult.Numbers.get(client, tedRadioNum, function (err, number) {
		tedRadio = number.name;
	});
	var serial;
	catapult.Numbers.get(client, serialNum, function (err, number) {
		serial = number.name;
	});
	var carTalk;
	catapult.Numbers.get(client, carTalkNum, function (err, number) {
		carTalk = number.name;
	});
	var gimlet;
	catapult.Numbers.get(client, gimletNum, function (err, number) {
		gimlet = number.name;
	});
	var radioLab;
	catapult.Numbers.get(client, radioLabNum, function (err, number) {
		radioLab = number.name;
	});
	var wtfPodcast;
	catapult.Numbers.get(client, wtfNum, function (err, number) {
		wtfPodcast = number.name;
	});
	var bsReport;
	catapult.Numbers.get(client, bsReportNum, function (err, number) {
		bsReport = number.name;
	});
	var securityNow;
	catapult.Numbers.get(client, securityNowNum, function (err, number) {
